      ...
      >
        ...
        <div className="flex flex-col md:flex-row justify-between md:items-end gap-12 mb-20 border-b border-white/10 pb-12">
          <div>
            ...
            <h2 className="text-[clamp(40px,5vw,72px)] font-sans font-light tracking-tight text-white uppercase flex flex-col leading-[0.9]">
              <span>Archive</span>
              <span className="font-['Instrument_Serif'] italic lowercase text-[#E55B13] tracking-normal">Collection</span>
            </h2>
          </div>

          <a 
            href="/" 
            className="group flex items-center gap-4 text-xs font-mono tracking-widest uppercase text-white hover:text-[#E55B13] transition-colors duration-300"
          >
            <div className="w-10 h-10 rounded-full border border-white/20 flex items-center justify-center group-hover:border-[#E55B13] group-hover:bg-[#E55B13]/10 transition-all duration-300">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-[1px] bg-white/10 border border-white/10 p-[1px] mb-20">
          {galleryItems.map((item, idx) => {
            ...
            // 2. ORANGE FOCUS RENDER
            if (item.type === 'orange') {
              return (
                <div key={item.id || `orange-${idx}`} className="bg-[#E55B13] aspect-square relative overflow-hidden p-8 flex flex-col justify-between group col-span-1">
                ...
                >
                  ...
                  <div className="absolute inset-0 p-6 flex flex-col justify-between z-20 pointer-events-none bg-gradient-to-t from-[#020202] via-black/20 to-transparent">
                    ...
                    <div className="translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 ease-out">
                      ...
                      <span className="text-[#E55B13] text-[9px] font-mono uppercase tracking-[0.2em]">{design.category}</span>
                    </div>
                  </div>
                </motion.div>
              );
