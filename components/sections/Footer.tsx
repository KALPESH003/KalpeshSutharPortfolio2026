...
// --- DYNAMIC SCRAMBLE TEXT EFFECT (Strict Time-Based Lottery Ticker) ---
const ScrambleText = ({ 
  text, 
  className, 
  trigger = "hover", 
  ...
  // Triggers when the element enters the viewport - Set to false so it happens every time
  const isInView = useInView(ref, { once: false, margin: "-50px" });

  const triggerGlitch = useCallback(() => {
    // Clear any existing interval to prevent overlapping glitches
    ...
    intervalRef.current = setInterval(() => {
      ...
      // Once we hit exactly 1 second (progress === 1), force the real text and clear
      if (progress === 1) {
        setDisplayText(text);
      ...
      setDisplayText(
        text
          .split('')
          .map((char, index) => {
            // Keep spaces intact to avoid layout jumping
            if (char === ' ') return ' ';
            
            // If the index is behind the time progress, lock in the real character
            if (index < lockIndex) {
              return text[index];
            }
            
            // Otherwise, show a random character like a lottery ticker
            return chars[Math.floor(Math.random() * chars.length)];
          })
          .join('')
      );
    }, 30); // ~33fps refresh rate for the scramble visual
  }, [text, duration]);
  ...
  }, [isInView, trigger, triggerGlitch]);
  ...
  return (
    <span
      ...
      // Apply pure white text while scrambling, revert to default class when done
      className={`inline-block transition-colors duration-300 ${isScrambling ? 'text-white' : ''} ${className || ''}`}
    >
      {displayText}
    </span>
  );
        ...
        >
          ...
          <div className="relative z-10 grid grid-cols-2 md:grid-cols-4 gap-y-12 gap-x-8 pt-12 md:pt-20">
            ...
            <div className="flex flex-col gap-4 justify-between h-full">
              ...
              <ScrambleText 
                text={`© ${new Date().getFullYear()} ${personalInfo?.name || "Kalpesh Suthar"}`} 
                trigger="inView"
                duration={1000}
                className="text-[10px] font-mono tracking-[0.2em] text-white/40 uppercase mt-auto cursor-default hover:text-white/70"
              />
            </div>
          </div>
